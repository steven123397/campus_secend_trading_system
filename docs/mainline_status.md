...
| M7 | 部署与在线访问地址 | 已完成 | 获得可访问公网网址并完成线上验证 | 已部署到 Render，公网地址为 `https://campus-secend-trading-system.onrender.com`，4 个页面线上验证通过 |
...
- 2026-04-13：完成 M7，已获得公网地址 `https://campus-secend-trading-system.onrender.com`；线上验证结果为 `/`、`/items`、`/users`、`/orders` 均可访问，首页显示商品总数 5、用户总数 4、订单总数 2，商品页查询 `unsold` 返回 3 条结果，用户页聚合查询 `item_total` 返回 1 条统计结果，订单页连接查询 `sold_with_buyer` 返回 2 条结果。
