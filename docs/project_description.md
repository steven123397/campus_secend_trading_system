...
图 1：首页整体效果，展示平台导航、宣传区、商品数量、用户数量和订单数量等概览信息。
...
图 2：登录页初始状态，用户可以输入用户编号或手机号以及密码进入系统。

![登录页截图](<image/4.2 登录页截图.png>)

图 3：登录成功后的页面状态，顶部导航显示当前登录用户信息，说明系统已经建立登录会话。
...
图 4：商品列表页面，展示数据库中商品表 `item` 的主要字段，并提供商品管理和查询入口。

![商品列表页面截图](<image/4.3 商品列表页面截图.png>)

图 5：新增商品运行结果，提交新增表单后，新商品被真实写入数据库并显示在商品列表中。

![新增商品运行结果截图](<image/4.3 新增商品运行结果截图.png>)

图 6：修改商品价格运行结果，提交改价表单后，目标商品价格刷新为修改后的值。

![修改商品价格运行结果截图](<image/4.3 修改商品价格运行结果截图.png>)

图 7：删除未售商品运行结果，系统只允许删除 `status = 0` 的未售商品，删除后列表同步更新。

![删除未售商品运行结果截图](<image/4.3 删除未售商品运行结果截图.png>)

图 8：购买商品运行结果，购买 CalculusBook 后系统生成订单，并将该商品状态更新为已售出。

![购买商品 CalculusBook 运行结果截图](<image/4.3 购买商品CalculusBook运行结果截图.png>)

图 9：重复购买拦截结果，已售出的 CalculusBook 再次购买时被系统阻止，保证每个商品最多交易一次。
...
图 10：用户列表页面，展示数据库中用户表 `user` 的主要数据，并提供商品统计类查询入口。
...
图 11：订单列表页面，展示订单表 `orders` 中的交易记录，并提供订单连接查询入口。
...
图 12：初始数据展示，商品列表页面可以看到题目要求的商品初始记录，说明种子数据已成功写入数据库。
...
图 13：插入新商品结果，新增商品操作完成后，页面刷新并展示新插入的商品记录。
...
图 14：修改商品价格结果，目标商品价格字段已经更新，证明修改操作真实作用于数据库。
...
图 15：删除未售商品结果，未售商品删除后不再出现在商品列表中。
...
图 16：购买商品成功结果，系统完成订单插入和商品状态更新，体现购买业务逻辑的事务执行结果。

![购买商品成功结果截图](<image/4.3 购买商品CalculusBook运行结果截图.png>)

图 17：已售商品再次购买失败结果，系统提示已售商品不能重复购买，体现唯一约束和业务校验生效。
